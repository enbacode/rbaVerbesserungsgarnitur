...
<b-card class="my-2" :header="roundObj.name" bg-variant="secondary" text-variant="white">
	...
	<template v-slot:footer>
		<b-form-row>
        <b-col sm="1"><big><b-icon-person-fill></b-icon-person-fill></big> {{ roundObj.mc }}</b-col>
        <b-col sm="1"><big><b-icon-cloud-download></b-icon-cloud-download></big> {{ roundObj.downloads }}</b-col>
        <b-col sm="2"><big><b-icon-calendar-fill></b-icon-calendar-fill></big> {{ roundObj.date.substring(0, 19) }}</b-col>
		<b-col><b-button :href="roundObj.mp3" variant="light" size="sm" class="float-right"><b-icon-download></b-icon-download> Download</b-button></b-col>
		</b-form-row>
	</template>
</b-card>
</template>
